...
setwd("~/Desktop/middletonLab/wildlife_recovery")

#temporary datasheet without 2021 case studies
datasheet <- fread("Mitch/edited_recovery2.csv")
...
##above from Mitch's code, will be added to main
academicAlluv <- Data %>% group_by(`Groups involved_Academic`, Goals_indicator, outcome_binary) %>%
  dplyr::summarise(Freq = n()) %>% ungroup() %>% filter(`Groups involved_Academic` == 1) %>%
  mutate(outcome_binary = ifelse(outcome_binary, "Positive", "Negative")) %>%
  mutate(group = "Academic") %>% dplyr::select(-`Groups involved_Academic`)

govtAlluv <- Data %>% group_by(`Groups involved_Government`, Goals_indicator, outcome_binary) %>%
  dplyr::summarise(Freq = n()) %>% ungroup() %>% filter(`Groups involved_Government` == 1) %>%
  mutate(outcome_binary = ifelse(outcome_binary, "Positive", "Negative")) %>%
  mutate(group = "Government") %>% dplyr::select(-`Groups involved_Government`)

localAlluv <- Data %>% group_by(`Groups involved_Local community/indigenous groups`, Goals_indicator, outcome_binary) %>%
  dplyr::summarise(Freq = n()) %>% ungroup() %>% filter(`Groups involved_Local community/indigenous groups` == 1) %>%
  mutate(outcome_binary = ifelse(outcome_binary, "Positive", "Negative")) %>%
  mutate(group = "Local community/indigenous groups") %>%
  dplyr::select(-`Groups involved_Local community/indigenous groups`)

nonProfitAlluv <- Data %>% group_by(`Groups involved_Non-profit`, Goals_indicator, outcome_binary) %>%
  dplyr::summarise(Freq = n()) %>% ungroup() %>% filter(`Groups involved_Non-profit` == 1) %>%
  mutate(outcome_binary = ifelse(outcome_binary, "Positive", "Negative")) %>%
  mutate(group = "Non-profit") %>%
  dplyr::select(-`Groups involved_Non-profit`)

privateCompanyAlluv <- Data %>% group_by(`Groups involved_Private company`, Goals_indicator, outcome_binary) %>%
  dplyr::summarise(Freq = n()) %>% ungroup() %>% filter(`Groups involved_Private company` == 1) %>%
  mutate(outcome_binary = ifelse(outcome_binary, "Positive", "Negative")) %>%
  mutate(group = "Private company") %>%
  dplyr::select(-`Groups involved_Private company`)

privateLandownersAlluv <- Data %>% group_by(`Groups involved_Private landowners`, Goals_indicator, outcome_binary) %>%
  dplyr::summarise(Freq = n()) %>% ungroup() %>% filter(`Groups involved_Private landowners` == 1) %>%
  mutate(outcome_binary = ifelse(outcome_binary, "Positive", "Negative")) %>%
  mutate(group = "Private landowners") %>%
  dplyr::select(-`Groups involved_Private landowners`)

zooAlluv <- Data %>% group_by(`Groups involved_Zoo`, Goals_indicator, outcome_binary) %>%
  dplyr::summarise(Freq = n()) %>% ungroup() %>% filter(`Groups involved_Zoo` == 1) %>%
  mutate(outcome_binary = ifelse(outcome_binary, "Positive", "Negative")) %>%
  mutate(group = "Zoo") %>%
  dplyr::select(-`Groups involved_Zoo`)

allAlluv <- rbindlist(list(academicAlluv, govtAlluv, localAlluv, nonProfitAlluv,
                           privateCompanyAlluv, privateLandownersAlluv, zooAlluv))

groupsAlluv <- ggplot(allAlluv,
       aes(y = Freq, axis1 = group, axis2 = Goals_indicator, axis3 = outcome_binary)) +
  geom_alluvium(aes(fill = group), width = 1/12) +
  geom_stratum(width = 1/12, fill = "black", color = "grey") +
  geom_label(stat = "stratum", aes(label = after_stat(stratum))) +
  scale_x_discrete(limits = c("Group", "Includes human dimension", "Outcome"), expand = c(.05, .05)) +
  scale_fill_brewer(type = "qual", palette = "Set1") +  theme_bw() + ylab("") + 
  theme(legend.position = "none", axis.text.y = element_blank(), axis.ticks = element_blank(),
        panel.grid.major = element_blank(), panel.grid.minor = element_blank(),
        panel.border = element_blank()) + scale_color_brewer(type = "qual")


basicAlluvData <- Data %>% group_by(Goals_indicator, outcome_binary) %>%
  dplyr::summarise(Freq = n()) %>%
  mutate(outcome_binary = ifelse(outcome_binary, "Positive", "Negative"))

basic <- ggplot(basicAlluvData,
       aes(y = Freq, axis1 = Goals_indicator, axis2 = outcome_binary)) +
  geom_alluvium(aes(fill = Goals_indicator), width = 1/12) +
  geom_stratum(width = 1/12, fill = "black", color = "grey") +
  geom_label(stat = "stratum", aes(label = after_stat(stratum))) +
  scale_x_discrete(limits = c("Includes human dimension", "Outcome"), expand = c(.05, .05)) +
  scale_fill_brewer(type = "qual", palette = "Set1") +  theme_bw() + ylab("") + 
  theme(legend.position = "none", axis.text.y = element_blank(), axis.ticks = element_blank(),
        panel.grid.major = element_blank(), panel.grid.minor = element_blank(),
        panel.border = element_blank()) + scale_color_brewer(type = "qual")
